...
<body>
    <div class="mobile-page">
        ...
        let roomId = ""
        let httpUrlApi = "http://127.0.0.1:8081"
        ...
        ws.onopen = function(evt) {
            ...
            //person =  getName();
            person = randomNumber(16125001, 16125004)
            console.log("用户准备登陆:" + person);
        ...
        ws.onmessage = function(evt) {
            ...
            if (data_array.cmd === "login") {
                loginData = JSON.parse(data_array.response.data)
                console.log("loginData", loginData)

                roomId = loginData.imRoomId
                //获取历史记录
            ...
            } else if (data_array.cmd === "receiveMsg") {
                console.log("msg receive", data_array.response.data)
                data = JSON.parse(data_array.response.data)
                addChatWith(msg(data.from, data.message))
            } else if (data_array.cmd === "enter") {

                data = JSON.parse(data_array.response.data)
                addChatWith(msg("管理员", "欢迎 " + data.formImAccount + " 加入~"))

                addUserList(data.formImAccount)
            } else if (data_array.cmd === "exit") {
                data = JSON.parse(data_array.response.data)

                addChatWith(msg("管理员", data.formImAccount + " 悄悄的离开了~"))
                delUserList(data.formImAccount)
            }
        };
